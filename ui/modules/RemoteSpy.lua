...
if not hasMethods(Methods.RequiredMethods) then return RemoteSpy end
...
function Condition.new(remote, status, index, value, type)
	...
	check:SetCallback(function() condition:Toggle() end)

	button:SetRightCallback(function() selected.condition = condition end)

	button:SetSelectedCallback(function()
		if not table.find(selected.conditions, condition) then table.insert(selected.conditions, condition) end
	end)

	if byType then instance.Identifiers.ByType.Visible = false end
...
function Log.new(remote)
	...
	local function viewLogs()
		if selected.remoteLog then remoteLogs:Clear() end
	...
	listButton:SetCallback(function()
		if selected.remoteLog ~= log then
			if #remote.Logs > 400 then
				MessageBox.Show("Warning", "This remote seems to have a lot of calls, opening this may cause your game to freeze for a few seconds.\n\nContinue?", MessageType.YesNo, viewLogs)
			else
				viewLogs()
			end
		end
	...
	listButton:SetSelectedCallback(function()
		if not table.find(selected.logs, log) then table.insert(selected.logs, log) end
	end)
...
function ArgsLog.new(log, callInfo)
	...
	if selected.remoteLog ~= log then instance.Visible = false end
...
function Log.playIgnore(log) log.IgnoreAnimation:Play() end

function Log.playBlock(log) log.BlockAnimation:Play() end

function Log.playNormal(log) log.NormalAnimation:Play() end
...
function Log.clear(log)
	...
	if selected.remoteLog == log then remoteLogs:Clear() end
...
ListRefresh.MouseButton1Click:Connect(function() refreshLogs() end)
...
LogsButtons.Clear.MouseButton1Click:Connect(function() selected.remoteLog:Clear() end)
...
ConditionsButtons.New.MouseButton1Click:Connect(function() newRemoteCondition:Show() end)

NewConditionButtons.Add.MouseButton1Click:Connect(function()
	if not conditionStatus.Selected then return MessageBox.Show("Error", "Invalid condition status", MessageType.OK) end
	...
	elseif valueType == "Value" then
		...
		elseif type == "number" then
			...
			if not value then return MessageBox.Show("Error", "Your input does not match the type you selected", MessageType.OK) end
...
NewConditionButtons.Cancel.MouseButton1Click:Connect(function() newRemoteCondition:Hide() end)
...
NewConditionIndex.Value.Input.FocusLost:Connect(function()
	...
	if not newIndex or newIndex <= 0 then NewConditionIndex.Value.Input.Text = 1 end
end)
...
clearContext:SetCallback(function() selected.logContext:Clear() end)
...
removeContext:SetCallback(function() selected.logContext:Remove() end)
...
unignoreContextSelected:SetCallback(function()
	for _i, log in pairs(selected.logs) do
		...
		if remote.Ignored then remote:Ignore() end
...
blockContextSelected:SetCallback(function()
	for _i, log in pairs(selected.logs) do
		...
		if remote.Blocked then remote:Block() end
...
scriptContext:SetCallback(function()
	...
	if selected.IsAnimeDefenders then script = "local oh = require(game:GetService('ReplicatedStorage').Actions)\n" end
	...
	if #selected.args == 0 then
		if selected.IsAnimeDefenders then
			setClipboard(script .. "oh." .. selected.AnimeDefendersRemote .. ":" .. "Fire" .. "()")
		else
...
viewAsHexContext:SetCallback(function()
	...
	if not selected.callPodButton.oldStrings then selected.callPodButton.oldStrings = {} end
